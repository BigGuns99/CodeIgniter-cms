...
class webconfig_helper extends Admin_Controller
{
    ...
    /**
     * @Description - Provide a multi-demensional array to parse and create a web.config file. The base setting are based on  the web.config examples at https://github.com/h5bp/server-configs 
    ...
    public function filter($query)
    {
        ...
        foreach ($query as $redirect) {
            $date = getdate(strtotime($redirect->created));
            $year = $date['year'];
            $month = strtolower($date['month']);
            $newLocation = $year . '/' . $month . '/' . $redirect->url;
            $redirects[] = array('orgUrl' => $redirect->redirect, 'redirect' => $newLocation
            );
        }
        return $redirects;
    }
