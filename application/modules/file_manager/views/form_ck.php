<?php
if (isset($method)) {
    echo form_open_multipart('file_manager/file_manager/update', array('enctype' => 'multipart/form-data', 'class' => 'form-horizontal'));
} else {
    echo form_open_multipart('file_manager/uploader/add_ck', array('enctype' => 'multipart/form-data', 'class' => 'form-horizontal'));
}
if (isset($method)) {
...
echo form_hidden('funcNum', $funcNum);
?>
<!-- NAME OF THE FILE --> 
            ...
            <select name="dir_id">
                <?php foreach ($directories as $parentDir): ?>
                    <?php if ($parentDir->id == $directory->id): ?>
                        <option value="<?php echo $parentDir->id; ?>" selected="<?php echo $directory->id; ?>"><?php echo $parentDir->title ?></option>
                    <?php else: ?>
...
<?php if (isset($method)): ?>
    ...
    <tbody>  
        ...
        <tr>
            <th>File Path</th>
            <td><?php echo $_SERVER['DOCUMENT_ROOT'] . $file->full_path; ?></td>
        </tr>
        <tr>
            <th>Url</th>
            <td><a href="<?php echo base_url($file->url_path); ?>"><?php echo base_url($file->url_path); ?></a></td>
                ...
                echo form_hidden('id', $file->id);
                ?>
                <button type="submit" id="submit"class="btn btn-danger"><i class="icon-trash"></i> Delete</button>
                <?php echo form_close(); ?>
</td>
